...
const current_playlist_position_display = document.querySelector('#currelaylist-position');
...
function load_abc_file(filename) {
    ...
    $.ajax({
        ...
        success: function (data) {
            let processed_abc = preprocess_abc(data);
            original_loaded_abc = processed_abc;
            loaded_abc_filename = filename;
            loaded_filename_display.textContent = filename;
            $('#abc-textarea').val(processed_abc);
            load_abc(processed_abc);
...
// Function to preprocess ABC file similar to send_abc_single
function preprocess_abc(abcText) {
    let lines = abcText.split('\n');
    let headers = [];
    let notes = [];
    
    const ignored_headers = new Set(['T', 'C', 'Z', 'S', 'N', 'G', 'O', 'H', 'I', 'P', 'W', 'F', 'B']);
    ...
    for (let line of lines) {
        ...
        if (line.length >= 2 && line[1] === ':' && /^[a-zA-Z]$/.test(line[0])) {
            if (ignored_headers.has(line[0].toUpperCase())) {
